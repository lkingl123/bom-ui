...
export async function POST(req: Request) {
  ...
  try {
    ...
    itemBoms.forEach((b, i) => {
      console.log(`   #${i + 1} → childProductId=${b.childProductId}, qty=${b.quantity.uomQuantity}`);
    });
    ...
    try {
      ...
      if (msg.includes("entity_modified") || msg.includes("409")) {
        console.warn("⚠️ Conflict detected (409/entity_modified). Retrying with updated timestamp...");
        const latest = await getProduct(productId, true);
        console.log("🕓 Latest product timestamp (after conflict):", latest?.timestamp);

        if (!latest) throw new Error("Failed to re-fetch product for retry.");

        const retryPayload = { ...payload, timestamp: latest.timestamp };
        console.log("🔁 Retrying PUT with new payload:", JSON.stringify(retryPayload, null, 2));
    ...
    console.error("💥 [products/update-boms] Fatal Error:", err);
    return NextResponse.json(
